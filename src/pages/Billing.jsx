...
export default function Billing() {
  ...
  useEffect(() => {
    const loadInvoices = async () => {
      ...
      const allPatientData = await Promise.all(
        patients.map((p) => patientService.getById(p.id))
      );

      const built = allPatientData.flatMap((patient) => {
        if (!patient) return [];
        return (patient.visits || []).map((visit) => {
          ...
          return {
            ...
            balance,
            status,
            date: visit.date ? new Date(visit.date).toLocaleDateString("en-CA") : "",
          };
        });
      });
  ...
  // ✅ async collect payment — no page reload
  const handleCollectPayment = async () => {
    ...
    if (amount > selectedInvoice.balance) {
      alert(`Maximum payment allowed is Rs. ${selectedInvoice.balance}`);
      return;
    }

    await patientService.collectPayment(
    ...
    await refreshInvoices(); // ✅ uses the non-effect version
  };
  ...
  const columns = [
    ...
    { key: "amount", label: "Amount", render: (row) => `Rs. ${row.amount.toLocaleString()}` },
    {
      key: "paidAmount", label: "Paid",
      render: (row) => <span className="text-green-600 font-medium">Rs. {row.paidAmount.toLocaleString()}</span>,
    },
    {
    ...
    { key: "date", label: "Date" },
    {
      ...
      render: (row) => (
        <div className="flex gap-2">
          <button
            onClick={() => navigate(`/patients/${row.patientId}/invoice`)}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 mt-4">
        {["All", "Paid", "Partial", "Pending"].map((item) => (
          ...
          >
            {item} ({item === "All" ? invoices.length : invoices.filter((i) => i.status === item).length})
          </button>
        ))}
      </div>
      ...
      {filteredInvoices.length ? (
        ...
        <EmptyState title="No Invoices Found" description="No invoice matches your search." />
      )}

      {selectedInvoice && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4">
            <h2 className="text-xl font-semibold mb-4">Collect Payment</h2>

            <div className="space-y-3 mb-6 bg-slate-50 p-4 rounded-xl">
              <p className="text-sm"><span className="font-medium">Patient:</span> {selectedInvoice.patient}</p>
              <p className="text-sm"><span className="font-medium">Treatment:</span> {selectedInvoice.treatment}</p>
              <div className="border-t border-slate-200 pt-3 mt-3 space-y-2">
                <p className="text-sm"><span className="font-medium">Total Amount:</span> Rs. {selectedInvoice.amount.toLocaleString()}</p>
                <p className="text-sm text-green-600"><span className="font-medium">Already Paid:</span> Rs. {selectedInvoice.paidAmount.toLocaleString()}</p>
                <p className="text-sm font-semibold text-red-600"><span className="font-medium">Remaining Balance:</span> Rs. {selectedInvoice.balance.toLocaleString()}</p>
              </div>
            </div>
