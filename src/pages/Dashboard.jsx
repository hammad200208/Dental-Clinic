...
export default function Dashboard() {
  ...
  const refreshAppointments = () => {
    setAllAppointments(appointmentService.getAll());
  };
  ...
  const topDebtors = patients
    .map((p) => {
      ...
      const paid = visits.reduce(
        (sum, v) => sum + (Number(v.paidAmount) || 0),
        0
      );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Dashboard"
        subtitle="Live clinic analytics system"
      />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border p-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Pending / Confirmed / Completed (same as before) */}
            {[["Pending", pending, "Confirmed"], ["Confirmed", confirmed, "Completed"], ["Completed", completed, null]].map(([label, list, nextStatus], i) => (
                  ...
                  list.map((a) => (
                    <div key={a.id} className="mb-3 border p-3 rounded-xl">
                      ...
                      {nextStatus && (
                        <button
                          onClick={() => {
                            appointmentService.updateStatus(a.id, nextStatus);
                            refreshAppointments();
                          }}
            ...
            {upcomingVisits.map((v, i) => (
              <div key={i} className="border p-3 rounded-xl mb-2">
                ...
                <p className="text-sm text-blue-600">
                  {v.nextVisit.toLocaleDateString()}
                </p>
              </div>
            ))}
          </div>
